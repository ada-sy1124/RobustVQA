...
def process_image(image: dict | Image.Image) -> Image.Image:
    ...
    if "bytes" in image:
        # --- 关键修改：在这里调用 Image.open()
        # 1. 封装 BytesIO
        bytes_io_obj = BytesIO(image["bytes"])
        
        # 2. 解码成 PIL Image 对象
        # 注意：这一步可能会触发我们之前解决的 DecompressionBombWarning
        # 确保在全局设置了 Image.MAX_IMAGE_PIXELS = None
        img_obj = Image.open(bytes_io_obj) 
        
        # 3. 将 PIL Image 对象放回字典或直接返回
        image["image"] = img_obj
