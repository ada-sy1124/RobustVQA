...
本项目围绕**电商客服退换货**场景，用强化学习对齐多模态大模型（MLLM）的推理行为：抑制**迎合用户情绪、脱离事实的「迎合性伪推理」**，缓解奖励稀疏，并提升逻辑自洽与可解释性。基座模型为 **Qwen3-VL-8B-Instruct**；训练侧基于开源 RL 框架 **[verl](https://github.com/volcengine/verl)**（HybridFlow）扩展。
